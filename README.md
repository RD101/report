...
- [x] 2020-07-01:
	...
	- csi진행상황 공유
		- 리뷰: 드로잉 영역 디버그. 스크린샷 이후 드로잉이 삭제되지 않던 버그 해결
...
- [x] 2020-08-19:
	...
	- csi진행상황 공유
		...
		- Review: ffmpeg, reviewdatapath 패스가 잘못되거나, 없는 경우 등등에 대한 체크코드를 추가함.
...
- [x] 2020-09-09: 코로나로 인한 비대면 회의 진행
	- 이슈 토론
	- 온라인 코드 리뷰
- [x] 2020-09-16: 도토리 프로젝트 마무리, 1년간의 진행 사항 전사 발표
	- gogs 이슈토론 및 로드맵 진행상황 체크
		- 한 주간 이슈 체크
		- ERP 진행상황 체크 및 컨설팅
		- 툴 도입에 대한 논의
	...
	- csi진행상황 공유
		- Publish: status를 수동으로 바꾸는 기능
		- Publish: modal 초기화 되도록 함.
		- Review: 작업자 이름이 자동으로 표기되도록 함.
		- 개발환경: assets_vfsdata.go는 버전관리에서 제외
		- Review: new window 모드 추가
		- Publish: REST API를 통해 publish 삭제하는 부분 버그 해결
		- Task: task 추가가 가능한 권한을 한 단계 낮춤
		- Review: 코멘트가 등록된 이후 창을 초기화시킴(터치로 조작할 때, 손의 작은 떨림으로 인해 코멘트가 중복등록되는 경우를 방지하기 위함)
		- Adminsetting: thumbnail path를 지정하는 옵션이 추가됨.
	- 에셋 라이브러리 툴(dotori)
		- 데모 준비 과정 공유
		- 발표 준비
		    - github의 Dotori 문서 정리
		    - https://github.com/RD101/dotori#dotori가제
	- 전사 발표
		- 요약 보고서
		- 도토리 요약 보고
		- 도토리 데모
